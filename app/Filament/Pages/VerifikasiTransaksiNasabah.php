...
class VerifikasiTransaksiNasabah extends Page implements HasActions
{
    use InteractsWithActions;

    protected static ?string $navigationIcon = 'heroicon-o-check-circle';
    protected static string $view = 'filament.pages.verifikasi-transaksi-nasabah';
    protected static ?string $navigationLabel = 'Verifikasi Transaksi Nasabah';
    protected static ?string $title = 'Verifikasi Transaksi Nasabah';
    protected static ?string $slug = 'verifikasi-transaksi-nasabah';
    ...
    public function loadTransaksi(): void
    {
        ...
        $this->transaksiList = $all->groupBy('penjemputan_id')->map(function ($items) {
            ...
            return [
                ...
                'total_pembayaran' => $items->sum('total_harga'),
    ...
    public function verifyPenjemputanAction(): Action
    {
        ...
        $totalPembayaran = $items->sum('total_harga');
        ...
        $detailHtml .= '<span class="font-semibold text-gray-900 dark:text-white">Total Pembayaran Nasabah:</span>';
        $detailHtml .= '<span class="font-bold text-lg text-green-600 dark:text-green-400">Rp ' . number_format($totalPembayaran, 0, ',', '.') . '</span>';
        $detailHtml .= '</div>';
        ...
        return Action::make('verifyPenjemputan')
            ->modalHeading('Verifikasi Pembayaran Nasabah')
                ...
                return new \Illuminate\Support\HtmlString(
                    '<div class="space-y-3">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Detail pembayaran dari <strong>Penjemputan #' . $this->selectedPenjemputanId . '</strong>:
                        </p>
                        ' . $detailHtml . '
                            ...
                            <div class="flex items-start gap-2">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="mt-1 text-sm text-yellow-700 dark:text-yellow-400">
                                        ✓ Pastikan file bukti sudah divalidasi sebelum menyetujui pembayaran.
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    public function verifyGroupByPenjemputan($penjemputanId): void
    {
        ...
        Notification::make()
            ->title('Berhasil')
            ->body('Semua pembayaran nasabah pada penjemputan #' . $penjemputanId . ' telah diverifikasi.')
            ->success()
            ->send();
    ...
    public function rejectGroupByPenjemputan($penjemputanId, $alasanPenolakan): void
    {
        ...
        Notification::make()
            ->title('Berhasil')
            ->body('Pembayaran nasabah pada penjemputan #' . $penjemputanId . ' telah ditolak!')
            ->success()
            ->send();
